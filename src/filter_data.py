...
def apply_model(X, y_onehot):
    ...
    clf = DecisionTreeClassifier(max_depth=None, min_samples_split=2, random_state=41)
    ...
    clf = RandomForestClassifier(
        n_estimators=10, max_depth=None, min_samples_split=2, random_state=41
    )
    scores = cross_val_score(clf, X, y_onehot, cv=5)
...
def plot_3D_PCA(df, FIG_DIR):
    ...
    principalComponents = pca.fit_transform(df.drop(["multi_label"], axis=1))
...
def get_selected_features(df):
    ...
    print(
        f"#rows after dropping duplicates and keeping first of the duplicates: \
    ...
    print(
        f"#rows after dropping ambiguous rows (same features in different classes): \
...
if __name__ == "__main__":
    ...
    feat_importances.nlargest(num_features).plot(
        kind="barh", title="Top {num_features} feature importances (ExtraTrees)"
    )
