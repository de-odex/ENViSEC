...
class Predict:
    ...
    def load_log2df(self):
        ...
        df.rename(
            columns=dict(zip(list(df.columns[0:-1]), list(df.columns[1:]))),
            inplace=True,
        )
    ...
    def refine_data(self, df):
        ...
        assert len(df_train.columns) == (
            len(df.columns) + len(label_cols)
    ...
    def predict_label(self):
        ...
        print(
            f"\t Benign Ratio: {(count_benign/(count_benign + count_attack)).round(2)* 100} %"
        )
        print("-" * 50)
        print("-" * 50)
        df_X_summary = df_X[
        ...
        df_attack = df_X_summary[df_X_summary.multi_label != "benign"].reset_index(
            drop=True
        )

        if len(df_attack) > 0:
            print(
                "Please check the following suspected devices (predicted as malicious):"
            )
            print("-" * 50)
            display(df_attack)
            self.predict_output = (
                self.predicted_path + "report_" + self.time_stamp + ".csv"
            )
