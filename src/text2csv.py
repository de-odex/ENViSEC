    ...
    for r, d, f in os.walk(DATA_DIR):
        for file in f:
            if file.endswith(".labeled"):
                ...
                print(
                    f"File: {file_name.rsplit('IoTScenarios', 2)[-1]} \
...
def file2rows(config, labeled_files, cols, chunk_size=1000000):
    ...
    for fc, file in enumerate(labeled_files):
        ...
        with open(file, "r") as fp:
            ...
            fpart = fpath.parts[fpath.parts.index("IoTScenarios") + 1]
            ...
            if config["preprocess"]["save_csv"] == "full":
                ...
                if count_rows % chunk_size > 0:
                    if count_rows < chunk_size:
                        ...
                        df_file = pd.concat([df_file, df_chunk], ignore_index=True)
                        print(
                            "Shape of the file before filering on merged chunks:",
                            df_file.shape,
                        )
...
if __name__ == "__main__":
    ...
    if config["debug"]:
        ...
        assert len(labeled_files) > 0, "No small files for debugging with size 5MB !"
