...
def filter_columns(df, txt_file=None):
    ...
    print(
        "Columns to remove from the dataset based on number of unique items \
        (1 or all unique):",
        cols_to_drop,
    )
...
def replace_values(df, cat_cols, data_process_for="train"):
    ...
    if "tunnel_parents" in df.columns:
        ...
        if data_process_for == "train":
            df = df[
                df.tunnel_parents.isin(["-", "(empty)"])
            ]  # filter other than '-' and '(empty)'.
        df["tunnel_parents"] = (
            df["tunnel_parents"].astype(str).str.strip().replace(dict_rep).astype(float)
        )
...
def render_plots(df):
    ...
    df.multi_label.value_counts().plot.pie(
        subplots=True,
        shadow=True,
        autopct="%1.1f%%",
        figsize=(6, 6),
...
def perform_norm(raw_data, normalize_file):
    ...
    if config["preprocess"]["include_history"]:
        ...
        df = df.drop("history", axis=1)  # remove history if disabled
