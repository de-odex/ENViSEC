...
def load_data_from_dir(data_dir):
    ...
    df = pd.DataFrame()
    if debug:
        df = pd.read_csv(filepath_or_buffer=csv_files[0], nrows=debug_rows)
    else:
        df = pd.concat(objs=[pd.read_csv(f) for f in csv_files], ignore_index=True)
...
def plot_history(fig_name, history):
    ...
    plt.plot(
        history.epoch, np.array(history.history["accuracy"]), label="train_accuracy"
    )
    plt.plot(
        history.epoch, np.array(history.history["val_accuracy"]), label="val_accuracy"
    )
